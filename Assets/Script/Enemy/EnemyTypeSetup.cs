using UnityEngine;

public class EnemyTypeSetup : MonoBehaviour
{
    public enum EnemyType { Type00 ,Type01, Type02, Type03, Type06, Type20, Type23, Type24, Type29,Type41, Type46, Type48 }

    public EnemyType type;

    private void Awake()
    {
        var brain = GetComponent<EnemyBrain>();
        if (brain == null) return;

        switch (type)
        {
            case EnemyType.Type00:
                brain.GetDecisionStageFunc = () => new DecisionStage_00(brain);
                break;

            case EnemyType.Type01:
                brain.GetDecisionStageFunc = () => new DecisionStage_1(brain);
                break;

            case EnemyType.Type02:
                brain.GetDecisionStageFunc = () => new IdleState(brain);
                break;

            case EnemyType.Type03:
                brain.GetDecisionStageFunc = () => new PatrolStage_03(brain);
                break;

            case EnemyType.Type06:
                brain.GetDecisionStageFunc = () => new DecisionStage_06(brain);
                break;

            case EnemyType.Type20:
                brain.GetDecisionStageFunc = () => new DecisionStage_20(brain);
                break;

            case EnemyType.Type23:
                brain.GetDecisionStageFunc = () => new DecisionStage_23(brain);
                break;

            case EnemyType.Type24:
                brain.GetDecisionStageFunc = () => new DecisionStage_24(brain);
                break;

            case EnemyType.Type29:
                brain.GetDecisionStageFunc = () => new DecisionStage_29(brain);
                break;

            case EnemyType.Type41:
                brain.GetDecisionStageFunc = () => new DecisionStage_41(brain);
                break;

            case EnemyType.Type46:
                brain.GetDecisionStageFunc = () => new DecisionStage_46(brain);
                break;

            case EnemyType.Type48:
                brain.GetDecisionStageFunc = () => new DecisionStage_48(brain);
                break;
        }
    }
}
